    ...
    >
      <div className="max-w-md w-full text-center rounded-2xl border border-zinc-800 bg-[#171a23] p-8 space-y-4">
        <div className="mx-auto h-12 w-12 rounded-full bg-[#c9a24c]/15 flex items-center justify-center">
          <AlertTriangle className="h-6 w-6 text-[#c9a24c]" />
        </div>
        <h2 className="text-lg font-semibold text-zinc-100">{title}</h2>
        <p className="text-sm text-zinc-400 leading-relaxed">{description}</p>
        <Link
          to="/auth"
          className="inline-flex items-center justify-center rounded-md border border-zinc-700 px-4 py-2 text-xs uppercase tracking-[0.2em] text-zinc-300 hover:border-[#c9a24c] hover:text-[#c9a24c] transition"
        >
          Ir para o login
        </Link>
      </div>
