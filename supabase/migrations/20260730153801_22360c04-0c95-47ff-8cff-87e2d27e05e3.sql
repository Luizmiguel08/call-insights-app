...
BEGIN
  ...
  -- Quem liga passa a ser o dono do contato (evita crédito trocado entre corretores)
  IF v_broker IS NOT NULL AND v_contact.broker_id IS DISTINCT FROM v_broker THEN
    -- Evita colisão com o índice único (broker_id, telefone) para linhas pendentes:
    -- marca eventuais cópias pendentes do mesmo telefone na fila deste corretor.
    UPDATE public.contacts_queue d
    SET status = 'duplicado'
    WHERE d.id <> _contact_id
      AND d.status = 'pending'
      AND d.broker_id = v_broker
  ...
  RETURN jsonb_build_object(
    'inserted', true,
    'already_resolved', v_already_resolved,
    'call_id', v_call_id,
    'contact_id', _contact_id,
    'attempts', v_attempts_after,
    'next', CASE WHEN v_next.id IS NULL THEN NULL ELSE to_jsonb(v_next) END
